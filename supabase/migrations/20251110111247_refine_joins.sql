...
AS $function$
  SELECT a.*,
    ...
    JSONB_AGG(DISTINCT to_jsonb(g)) AS data_points
FROM user_profile a
  LEFT JOIN(SELECT pd.*,
  ...
  LEFT JOIN bounties c ON c.parent_id = a.id
  LEFT JOIN bounties_join_table d ON d.user_id = a.id
  LEFT JOIN bounties e ON e.id = d.bounty_id
  LEFT JOIN achievements f ON f.profile_id = a.id 
  LEFT JOIN data_points g ON g.child_id = a.id
